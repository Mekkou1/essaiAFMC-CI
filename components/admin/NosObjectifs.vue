<template>
  <section id="objectives" class="py-5" style="text-align: center;">
    ...
    <div class="container" style="padding-top: 10px;">
        ...
        >
          ...
          >
            <div class="card-body text-center p-4">
              ...
              <button class="btn btn-warning mt-3" @click="editImage(index)">
                Modifier l'image
              </button>
            </div>
          </div>
      ...
      <div v-if="editingIndex !== null" class="edit-image-form">
        <h3>Modifier l'image pour {{ objectivesGroups[editingIndex]?.title }}</h3>
...
export default {
  ...
  methods: {
    ...
    saveImage () {
      if (this.newImageFile && this.editingIndex !== null) {
        const group = this.objectivesGroups[this.editingIndex]
        // eslint-disable-next-line no-unused-vars
        const oldImage = this.backgroundImages[this.editingIndex]

        // Logique pour déplacer la nouvelle image dans le dossier statique
        const newImagePath = `static/images/${this.newImageFile.name}`

        // Mettez à jour le lien de l'image dans le fichier de données
        group.image = newImagePath
        this.backgroundImages[this.editingIndex] = newImagePath

        // Logique pour supprimer l'ancienne image si nécessaire
        // Vous pouvez ajouter ici le code pour supprimer l'ancienne image

        // Réinitialiser les champs
        this.newImageFile = null
        this.editingIndex = null

        // Utilisez la méthode pour mettre à jour le fichier objectivesData ici si nécessaire
      }
    },
    cancelEdit () {
      this.newImageFile = null
  ...
  .edit-image-form {
    margin-top: 20px;
    text-align: left; /* Alignement à gauche pour le formulaire */
  }
